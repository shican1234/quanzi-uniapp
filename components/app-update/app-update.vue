		...
		methods: {
			...
			getUpdateInfo() {
				...
				updateInfo(data).then(res => {
					console.log(res,"3231")
					// 循环获取当前设备对应的更新数据
					for (let i = 0; i < res.length; i++) {
						if (vm.platform == res[i]['os']) {
							vm.update_info = res[i];
							break;
						}
					}
			...
			checkUpdate() {
				...
				if (vm.need_update) {
					...
					uni.setStorageSync("isUpdateShow",true)
			...
			closeUpdate() {
				if (vm.update_info.forces == 1) {
					...
					uni.setStorageSync("isUpdateShow",false)
				}
			},
			// 立即更新
			nowUpdate() {
