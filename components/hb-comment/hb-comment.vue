					...
					<view class="comment-main">
						...
						<view class="comment-main-top">
							...
							<view class="zan-box" @click="like(item.id)">
								<span :class="item.hasLike == 1 ? 'isLike' : 'notLike'">{{item.likeNum == 0 ? '抢首赞' : item.likeNum}}</span>
								<img style="width: 14px; height: 14px;" v-if="item.hasLike == 0"
						...
						<view class="comment-main-foot">
							...
							<view class="foot-btn" v-if="item.owners == 1" @click="confirmDelete(item.id)">删除</view>
						</view>
						<!-- 父评论体-end -->
						<!-- 子评论列表-start -->
						<view class="comment-sub-box">
							<view class="comment-sub-item" v-for="(each, index)  in item.children" :key="index">
								<view @click="toUserInfo(each.commentUserId)">
									<image :src="each.avatarUrl || emptyAvatar" mode="aspectFill" class="avatar">
									</image>
								</view>
								<view class="comment-main">
									<view class="sub-comment-main-top">
										<view class="nick-name" @click="toUserInfo(each.commentUserId)">{{each.nickName}}</view>
										<view class="zan-box" @click="like(each.id)">
											<span :class="each.hasLike == 1 ? 'isLike' : 'notLike'">{{each.likeNum == 0 ? '抢首赞' : each.likeNum}}</span>
											<img style="width: 14px; height: 14px;" v-if="each.hasLike == 0"
												src="data:image/png;base64," />
											<img style="width: 14px; height: 14px;" v-else
												src="data:image/png;base64," />
										</view>
									...
									<view class="comment-main-foot">
										...
										<view class="foot-btn" @click="reply(item.nickName,each.nickName,item.id)">
											回复</view>
										<view class="foot-btn" v-if="each.owners == 1" @click="confirmDelete(each.id)">删除
										</view>
									</view>
								</view>
							</view>
			...
			likeComplete(commentId) {
				for (var i in this.commentData.comment) {
					if (this.commentData.comment[i].id == commentId) {
						this.commentData.comment[i].hasLike == 1 ? this.commentData.comment[i].likeNum-- : this.commentData
							.comment[i].likeNum++;
						this.commentData.comment[i].hasLike == 1 ? this.commentData.comment[i].hasLike = 0 :this.commentData.comment[i].hasLike = 1
						return
					}
					for (var j in this.commentData.comment[i].children) {
						if (this.commentData.comment[i].children[j].id == commentId) {
							this.commentData.comment[i].children[j].hasLike == 1 ? this.commentData.comment[i].children[j]
								.likeNum-- : this.commentData.comment[i].children[j].likeNum++;
							this.commentData.comment[i].children[j].hasLike  == 1 ? this.commentData.comment[i].children[j]
								.hasLike = 0 :this.commentData.comment[i].children[j]
								.hasLike = 1
							return
						}
					}
