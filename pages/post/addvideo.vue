<template>
	...
	<view class="release">
		...
		<view class="wrap-card">
			...
			<u-upload ref="upload" accept="video" :maxSize="10 * 1024 * 1024" :maxDuration="180" :fileList="imgList" @afterRead="autoUpload" @delete="deletePic" @oversize="oversize" name="1" multiple
...
<script>
	import {uplodaFiles,addPostByVideo} from '@/utils/lib/api.js';  //引入需要的api
	export default {
		data() {
			return {
				textContent: '',
		...
		methods: {
			...
			submit() {
				...
				addPostByVideo(this.params).then(data => {
					uni.showToast({
					    icon: 'success',   
					    duration: 1000,
					    title: '发布成功!'
